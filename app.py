...
with r1_2: min_win_prob = st.slider("🎯 AI 預測勝率門檻 (%)", 40, 90, 50, step=5)
with r1_3: min_rvol = st.number_input("🔥 RVOL 爆發動能 (>1為爆量)", min_value=0.5, value=1.1, step=0.1)

st.subheader("🎯 第二階段：VWAP 狙擊濾網 (Micro Filters)")
r2_1, r2_2 = st.columns(2)
with r2_1: require_vwap = st.checkbox("☑️ 嚴格要求現價站上 VWAP (主力成本線)", value=False)
...
if st.button("🚀 啟動 AI x VWAP 雙階段掃描"):
    ...
    if len(stage1_passed) > 0:
            ...
            for stock in stage1_passed:
                try:
                    ...
                    if len(ticks) > 0:
                        ...
                        if require_vwap and not is_above_vwap: pass 
                        else:
        ...
        if final_results:
            display_df = pd.DataFrame(final_results).rename(columns={
